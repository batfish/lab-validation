...
**ATOMIC_AGGREGATE** attribute. It does _not_ form an AS_SET union of the
...
#10094 documents that the contributor AS _is_ carried on a single-origin
aggregate (`65001` present), which Batfish models as an empty AS_PATH. This
lab pins down the multi-contributor case that #10094 left open: the carried
path is the common leading sequence, not a set union, and divergent ASes
are dropped under atomic-aggregate. A Batfish fix should reproduce the
common-leading-sequence + atomic-aggregate behavior, not an AS_SET.
