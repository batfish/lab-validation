...
resource "aws_route_table_association" "rtb-asctn-igw-private-bat" {
  route_table_id = aws_route_table.bat_rtb_igw.id
...
resource "aws_subnet" "fish_private_subnet" {
  vpc_id     = aws_vpc.fish.id
  cidr_block = var.fish_private_subnet
  availability_zone = data.aws_availability_zones.az.names[0]
  tags = {
    Name = format("%s_%s", var.fish, var.fish_private_subnet)
  }
}
...
resource "aws_route_table_association" "rtb-asctn-igw-private-fish" {
  route_table_id = aws_route_table.fish_rtb_igw.id
  subnet_id      = aws_subnet.fish_private_subnet.id
}
############# security-group #############
# Define security-group
...
resource "aws_network_acl" "fish_nacl" {
  ...
  subnet_ids = [
    ...
    aws_subnet.fish_private_subnet.id
  ]
  ingress {
    protocol   = "-1"
    rule_no    = 1000
