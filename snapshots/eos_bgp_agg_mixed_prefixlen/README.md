...
Same root cause as `eos_bgp_agg_as_path`: Batfish had modeled the aggregate
with an empty AS_PATH, differing from the device on the aggregator, the
downstream neighbor, and the transit/contributor ASes (which reject the
re-advertised aggregate as an AS-path loop once it carries 65000). Batfish
now reproduces the common-leading-sequence behavior (#10094 fixed); the
remaining sickbay covers only the aggregator's local-weight difference
(#152).
